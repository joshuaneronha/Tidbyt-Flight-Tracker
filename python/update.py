...
try:
    ...
    for index in flights:
        next_flight = flights[index]
        if time.mktime(parser.parse(next_flight['datetime']).timetuple()) > time.time():
    ...
    next_flight['short_time'] = next_flight['datetime'][-5:]

    next_flight['flight_number'] = next_flight['flight_iata'][2:]
    ...
    if next_flight['dep_gate'] != None:
        next_flight['marquee'] = next_flight['short_time'] + '     ' + next_flight['aircraft'] + '     Gate ' + str(next_flight['dep_gate']).split('.')[0]
    else:
        next_flight['marquee'] = next_flight['short_time'] + '     ' + next_flight['aircraft']
